...
class DatastoreUserDBOperationMixin(object):
  ...
  def create_inactive_user(cls, user_name, password, email, send_email=True,
    ...
    def txn():
      key_name = cls.get_key_name(email)
      user = cls.get_by_key_name(email)
      if user:
        from yan.auth import DuplicateKeyError
        raise DuplicateKeyError(_(u"This email is already registered."
                                  " Please choose another email."))
      if do_registration:
        salt = crypto.gen_salt()
        activation_key = crypto.sha1(salt+email).hexdigest()
