...
class DatastoreBackendWithReverseProxiedDomainHack(DatastoreBackend):

  def create_login_url(self, url):
    hostname = settings.REVERSE_PROXIED_DOMAIN
    parts=urlparse(url)
    return url_for("auth/login", next=url_quote_plus(urlunparse((parts[0],hostname,parts[2],parts[3],parts[4],parts[5]))))

  def create_logout_url(self, url):
    hostname = settings.REVERSE_PROXIED_DOMAIN
    parts=urlparse(url)
    return url_for("auth/logout", next=url_quote_plus(urlunparse((parts[0],hostname,parts[2],parts[3],parts[4],parts[5]))))
